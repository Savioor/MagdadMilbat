...
public static final String TABLE_NAME = "Training";
    ...
    private void createTrainingTable(SQLiteDatabase sqLiteDatabase)
    {
        String sql = "Create table Training (id integer primary key autoincrement, exerciseDescription text not null," +
                " trainingQuality integer not null, date text not null, time text not null, duration double not null, repsDuration text not null, repsMaxHeight text not null, balldata integer not null)";
        sqLiteDatabase.execSQL(sql);
    }

    /** This function adds the value of the training it receives to the training table **/
    public void addTraining(Training training) {
        String sql = "insert into Training(date, time, exerciseDescription, trainingQuality, duration, repsDuration, repsMaxHeight, balldata) " +
                "values ('"+training.getDate() + "','" + training.getTime() + "','" + training.getExerciseDescription() + "','"+ training.getTrainingQuality() + "','"+ training.getDuration() + "','"+training.getRepDuration()+"','" +training.getRepMaxHeight()+"','"+training.getBalldata()+"')";
...
**/
    private Training getTraining(Cursor cursor)
    {
        ...
        return new Training(date, time, exerciseDescription, trainingQuality,duration,repsDuration,repsMaxHeight,balldata);
    }
